...
import { ArrowLeft, Search, FileText, ChevronRight, Plus } from 'lucide-react'
  ...
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col">
      <header className="bg-white border-b border-slate-200 px-4 py-3 flex items-center gap-4 sticky top-0 z-40">
        <Link href="/versicherung" className="w-10 h-10 rounded-xl bg-slate-100 flex items-center justify-center">
        ...
        <Link href="/versicherung/schaden-melden" className="w-10 h-10 rounded-xl bg-purple-500 flex items-center justify-center">
      ...
      <main className="flex-1 p-4 pb-8">
        <div className="relative mb-4">
          ...
          <input
            type="text"
            placeholder="Name, Kennzeichen, Werkstatt, Auftragsnr..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full bg-white rounded-xl pl-12 pr-4 py-3 border border-slate-200 text-sm focus:border-purple-400 focus:ring-2 focus:ring-purple-100 outline-none"
          />
        </div>
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 scrollbar-hide">
          <button
            ...
            className={`px-4 py-2 rounded-xl text-sm font-medium whitespace-nowrap ${
              filter === 'alle' ? 'bg-purple-500 text-white' : 'bg-white text-slate-600 border border-slate-200'
            }`}
          >
            Alle
          </button>
              ...
              return (
                ...
                >
                  <div className="flex items-start justify-between mb-2">
                    <div>
                      <p className="font-mono text-xs text-purple-600 font-medium mb-1">{claim.auftragsnummer || '-'}</p>
                  ...
                  <div className="flex items-center justify-between text-sm">
                    <span className="text-slate-500">{claim.werkstatt_name || 'Keine Werkstatt'}</span>
