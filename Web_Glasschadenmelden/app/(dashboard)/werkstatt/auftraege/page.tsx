...
export default function WerkstattAuftraegePage() {
  ...
  const [werkstattId, setWerkstattId] = useState<string | null>(null)
  ...
  async function checkAuth() {
    ...
    if (werkstattData) {
      setWerkstattId(werkstattData.id)
      loadClaims(werkstattData.id)
    }
  }

  async function loadClaims(wId: string) {
    // Get standorte first
  ...
  const filteredClaims = claims.filter(claim => {
    ...
    const matchesSearch = searchTerm === '' ||
      ...
      claim.vermittler_firma?.toLowerCase().includes(searchLower) ||
            ...
            <div className="flex-1 min-w-0">
              <div className="relative">
                ...
                <input
                  ...
                  className="w-full h-12 pl-12 pr-4 rounded-xl border border-slate-200 bg-white text-sm focus:border-orange-400 focus:ring-2 focus:ring-orange-100 outline-none transition-all"
                />
              </div>
            </div>
            {/* Status Filter */}
            <div className="flex gap-2 flex-wrap flex-shrink-0">
              <button
                ...
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-colors ${
                  filter === 'alle' ? 'bg-orange-500 text-white' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'
                }`}
              >
                Alle
              </button>
                    ...
                    return (
                      <tr key={claim.id} className="hover:bg-slate-50 transition-colors">
                        <td className="px-6 py-4">
                          <span className="font-mono text-sm font-medium text-orange-600">{claim.auftragsnummer || '-'}</span>
                        </td>
                        <td className="px-6 py-4">
                          <div>
